...
public class BooksDetails extends JFrame implements ActionListener {
	...
	public BooksDetails() {
		...
		search.setFont(new Font("Tebuchet MS", Font.BOLD | Font.ITALIC, 17));
		...
		l3.setFont(new Font("Trebichet MS", Font.BOLD, 18));
		...
		panel.setBorder(new TitledBorder(new LineBorder(new Color(0, 128, 128), 3, true),"Books Details", TitledBorder.LEADING,
	...
	public void actionPerformed(ActionEvent ae) {
		try {
			Conn con = new Conn();
			if(ae.getSource() == b1) {
				String sql = "select * from book where concat(name, student_id) like ?";
				PreparedStatement st = con.c.prepareStatement(sql);
				st.setNString(1, "%" + search.getText() + "%");
				ResultSet rs = st.executeQuery();
